function transform(events) {
  ...
  const ANDROID_USER_AGENT =
    "Dalvik/2.1.0 (Linux; U; Android 9; Mi A3 Build/PKQ1.190416.001)";
  const IOS_PRESENT_UA = "ios";

  // const destination = events[0].destination;

  const GAbatchLengthToSend = 5; // set whatever size you want to set in the reduced batch
  ...
  events.map(event => {
    ...
    event.properties.category = event.properties.category
      ? event.properties.category
      : eventName;
    const { userAgent } = event.context;
    if (
      userAgent === undefined ||
      userAgent === null ||
      userAgent.length === 0
      ...
      event.context.userAgent =
        platform === "iOS" ? IOS_USER_AGENT : ANDROID_USER_AGENT;
    }
    ...
    const ups = Object.keys(event.userProperties);
    ups.forEach(up => {
  ...
  const shuffledList = events.sort(() => 0.5 - Math.random());
