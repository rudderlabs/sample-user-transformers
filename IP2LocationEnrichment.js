    ...
    events.map(async e => {
      if (e.request_ip) {
        ...
        const res = await fetch(
          `https://api.ip2location.com/v2/?ip=${e.request_ip.trim()}&addon=<required addon e.g.geotargeting>&lang=en&key=<IP2Location_API_Key>&package=<package as required e.g. WS10>`
        );
        e.geolocation = res;
      }
